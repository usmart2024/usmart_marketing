...
Future maybeCreateUser(User user) async {
  final userRecord = BookmarksRecord.collection.doc(user.uid);
  final userExists = await userRecord.get().then((u) => u.exists);
  if (userExists) {
    currentUserDocument = await BookmarksRecord.getDocumentOnce(userRecord);
    return;
  }

  final userData = createBookmarksRecordData(
    email: user.email ??
  ...
  currentUserDocument =
      BookmarksRecord.getDocumentFromData(userData, userRecord);
}

Future updateUserDocument({String? email}) async {
  await currentUserDocument?.reference
      .update(createBookmarksRecordData(email: email));
}
